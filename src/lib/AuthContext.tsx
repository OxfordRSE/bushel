...
export const AuthProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const [impersonationTarget, setImpersonationTarget] =
    useState<FigshareUser | null>(null);
  const queryClient = useQueryClient();

  // Patch GET requests to support Auth and Impersonation
  //
  // Note: Uses query parameter authentication (access_token=...) to avoid CORS preflight.
  // Authorization header would trigger OPTIONS preflight, which Figshare's API doesn't
  // properly support (returns 204 with missing CORS headers).
  //
  // For endpoints that REQUIRE Authorization header (like institutional admin endpoints),
  // use a backend proxy instead (see /api/institution/accounts).
  ...
  const patchedFetch = useCallback(
    async <T,>(
      url: string | URL,
      options?: Omit<RequestInit, "body"> & { body?: object },
      internal_options?: { returnRawResponse: boolean },
  ...
  const licences = useQuery({
    ...
    enabled: !!token,
    queryFn: async () => {
      return await patchedFetch<FigshareLicense[]>("/api/account/licenses");
    },
  });
  ...
  const categories = useQuery({
    ...
    enabled: !!token,
    queryFn: async () => {
      return await patchedFetch<FigshareCategory[]>("/api/account/categories");
    },
  });
