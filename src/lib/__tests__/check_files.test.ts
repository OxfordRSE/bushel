...
class MockFileSystemFileHandle implements FileSystemFileHandle {
  kind = "file" as const;
  constructor(public name: string, private readonly data: File) {}
  isSameEntry(): Promise<boolean> {
    return Promise.resolve(true);
  }
...
class MockFileSystemDirectoryHandle implements FileSystemDirectoryHandle {
  ...
  getDirectoryHandle(): Promise<FileSystemDirectoryHandle> {throw new Error("Not implemented")}
  removeEntry(): Promise<void> {throw new Error("Not implemented")}
  resolve(): Promise<string[] | null> {throw new Error("Not implemented")}
}

beforeEach(() => {
  (globalThis as unknown as { FileSystemHandle: unknown }).FileSystemHandle = MockFileSystemHandle;
  (globalThis as unknown as { FileSystemFileHandle: unknown }).FileSystemFileHandle = MockFileSystemFileHandle;
});
...
describe('fileRefCheck', () => {
  ...
  it('fails if browser API is not supported', async () => {
    delete (globalThis as unknown as {FileSystemHandle: never}).FileSystemHandle;
  ...
  it('warns on empty file', async () => {
    ...
    const rootDir = new MockFileSystemDirectoryHandle('rootdir', { 'empty.txt': file });
    ...
    expect(emit).toHaveBeenCalledWith(expect.objectContaining({
      status: 'in_progress',
      warning: expect.stringMatching(/empty/i)
    }));
  });

  it('tracks quota when file has size', async () => {
    const file = new File(['abc'], 'nonempty.txt');
    const rootDir = new MockFileSystemDirectoryHandle('rootdir', { 'nonempty.txt': file });
    ...
    expect(emit).toHaveBeenCalledWith(expect.objectContaining({ status: 'success' }));
  });
  ...
  it('emits error if file not found', async () => {
    ...
    await fileRefCheck.run(parser as never, emit, { rootDir });
    const err = emit.mock.calls.find(([a]) => a.error instanceof DataError)?.[0];
    expect(err?.error.kind).toBe('FileNotFound');
  });

  it('emits error if filename is not a string', async () => {
    const file = new File(['hi'], 'good.txt');
    const rootDir = new MockFileSystemDirectoryHandle('root_dir', { 'good.txt': file });
