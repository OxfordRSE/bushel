...
export default function UploadStep({
  ...
  const all_rows_valid = useMemo(() => {
    return (
      parsedRows.every((row) => row.status === "valid") &&
      fileChecks.every((check) => check.status === "valid")
    );
  ...
  const overallStatus = useMemo(() => {
    ...
    if (rows.every((r) => r.status === "completed" || r.status === "skipped"))
      return "complete";
  ...
  const summary = {
    error: {
      title: "Error uploading to FigShare",
      status: "error" as Parameters<typeof StepPanel>[0]["status"],
      icon: <TriangleAlertIcon className="text-red-600" />,
    },
    "in progress": {
      title: "Uploading...",
      status: "default" as Parameters<typeof StepPanel>[0]["status"],
      icon: <CogIcon className="h-5 w-5 animate-spin text-blue-600" />,
    },
    complete: {
      title: "Upload complete!",
      status: "complete" as Parameters<typeof StepPanel>[0]["status"],
    },
    pending: {
  ...
  if (!all_rows_valid) {
    return (
      <StepPanel
        title="Upload to FigShare"
        iconOverride={<Ban className="h-5 w-5 text-gray-400" />}
        openByDefault={openByDefault}
      >
        <p>
          All data must be successfully parsed before it can be uploaded to
          FigShare.
        </p>
      </StepPanel>
    );
  ...
  if (
    !duplicatesAcknowledged &&
    (exactMatches.length || fuzzyWarnings.length)
  ) {
    return (
      <StepPanel
        title="Upload to FigShare"
      ...
      >
        <p>
          ...
          Please check the previous step and acknowledge that you understand the
          implications of uploading these articles.
        </p>
      </StepPanel>
    );
  }
  ...
  const rowStatus = (row: UploadRowStateWithTitle) => {
    if (row.fileProgress?.error) {
      return (
        <TooltipProvider>
          <Tooltip>
            <TooltipTrigger asChild>Upload error</TooltipTrigger>
            <TooltipContent side="top">{row.fileProgress.error}</TooltipContent>
          </Tooltip>
        </TooltipProvider>
      );
    }
      ...
      return (
        <TooltipProvider>
          <Tooltip>
            ...
            <TooltipContent side="top">{row.error?.message}</TooltipContent>
          </Tooltip>
        </TooltipProvider>
      );
    }
  ...
  return (
    ...
    >
      <div className="space-y-4">
        <p>
          Ready to upload <strong>{rows.length}</strong> articles. Please make
          sure {impersonationTarget?.first_name ?? "you"} have enough headroom
          to create them.
        </p>
            ...
            <TableBody>
              {displayedRows.map((row) =>
                exactMatches.includes(row.title ?? "") ? (
                  <TableRow key={row.id} className="bg-gray-50 opacity-25">
                    ...
                    <TableCell>
                      {row.fileProgress?.totalFiles ? (
                        <div className="flex items-center gap-1">
                          {Array.from({
                            length: row.fileProgress.totalFiles,
                          }).map((_, i) => {
                                ...
                                <Tooltip>
                                  ...
                                  <TooltipContent side="top">
                                    {isActive && fp.error ? (
                                      ...
                                      <>
                                        Uploading <strong>{fp.name}</strong>
                                        <br />
              ...
              {rows.length <= visibleRowCount &&
                rows.length > ROW_SUMMARY_LENGTH && (
                  <TableRow>
                    <TableCell colSpan={5} className="text-center">
                      <Button
                        variant="outline"
